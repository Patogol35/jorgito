...
export default function LinuxTerminal({ t, lang, onClose }) {
  ...
  const executeCommand = (raw) => {
    ...
    switch (cmd) {
      case "help":
        output = (
          <Box>
            ...
            <Box sx={{
              display: "grid",
              gridTemplateColumns: { xs: "100px 1fr", sm: "180px 1fr" },
              gap: "5px 15px",
            }}>
              {commands.map((item) => (
      ...
      case "ls":
        output = currentPath === "/home/jorge"
          ? "about.txt    skills.txt    projects    contact.txt"
          : [
              "weather-app.txt", "ecommerce.txt", "ai-chess.txt",
              "quiz.txt", "calculator.txt", "chatbot.txt",
            ].join("    ");
        break;
      ...
      case "cd": {
        ...
        else if (target === ".." && currentPath === "/home/jorge/projects")
          setCurrentPath("/home/jorge");
        else if (target === "projects" && currentPath === "/home/jorge")
          setCurrentPath("/home/jorge/projects");
        else if (["home", "jorge", "/home/jorge"].includes(target))
          setCurrentPath("/home/jorge");
        else
      ...
      case "cat": {
        ...
        else if (["about.txt", "skills.txt", "contact.txt"].includes(file))
          output = text(`${file.replace(".txt", "")}Content`);
        else
          output =
        ...
        output = (
          <Box>
            {terminal?.messages?.projectFiles?.map((file) => (
              <Typography key={file} sx={{ color: green, fontFamily: mono, lineHeight: 1.8 }}>
                {file}
              </Typography>
            ))}
          </Box>
      ...
      case "history":
        output = commandHistory.map((item, i) => (
          <Typography key={`${item}-${i}`} sx={{ color: muted, fontFamily: mono, lineHeight: 1.7 }}>
            {i + 1} {item}
          </Typography>
        ));
        break;
      ...
      case "date":
        output = new Date().toLocaleString(lang === "es" ? "es-ES" : "en-US");
        break;

      case "neofetch":
        output = (
          <Box sx={{ whiteSpace: "pre-wrap", fontFamily: mono, lineHeight: 1.5 }}>
{`       .--.
      |o_o |
      |:_/ |
     //   \\ \\
...
${terminal?.messages?.neofetch?.language || "Language:"} ${lang === "es" ? "Español" : "English"}`}
          </Box>
        );
        break;
  ...
  return (
    <Box sx={{ width: "min(100%, 1100px)", maxWidth: "100%", mx: "auto", p: { xs: 0, sm: 1 } }}>
      <Box sx={{
        width: "100%",
        overflow: "hidden",
        ...
        <Box sx={{
          ...
          <IconButton onClick={onClose} size="small" aria-label="Cerrar terminal"
        ...
        <Box
          ...
          sx={{
            width: "100%",
            height: { xs: "calc(100vh - 170px)", sm: 560 },
            minHeight: 300,
            overflowY: "auto",
            overflowX: "hidden",
        ...
        >
            ...
            {quickCommands.map(([command, label]) => (
              <Chip
                ...
                sx={{
                  ...
                  fontFamily: mono,
                  "&:hover": { bgcolor: "rgba(126,231,135,0.12)" },
                }}
              />
            ))}
          </Stack>
          ...
          {history.map((item, index) =>
            item.type === "command" ? (
              <Box key={index} sx={{ mt: 1, overflowWrap: "anywhere", wordBreak: "break-word" }}>
                ...
                <Typography component="span" sx={{ color: "#fff", fontFamily: mono, ml: 0.5 }}>
                  $ {item.command}
                </Typography>
              </Box>
            ) : (
          ...
          <Box component="form" onSubmit={handleSubmit}
            sx={{ display: "flex", alignItems: "flex-start", mt: 1, width: "100%" }}>
            <Typography component="span" sx={{ color: green, fontFamily: mono }}>
              jorge@portfolio:
            </Typography>
            <Typography component="span" sx={{ color: blue, fontFamily: mono, ml: 0.5 }}>
              {currentPath}
            </Typography>
            <Typography component="span" sx={{ color: "#fff", fontFamily: mono, ml: 0.5, mr: 0.8 }}>
              $
            </Typography>
        ...
        <Box sx={{
          ...
          <Typography sx={{
            ...
            {terminal?.hints || "↑ ↓ history · Tab autocomplete · Enter execute"}
          </Typography>
